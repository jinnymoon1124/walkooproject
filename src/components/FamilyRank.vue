<template>
  ...
  <div>
    ...
    <div class="arrow-buttons-container">
      <!-- 왼쪽 화살표 버튼 -->
      <button @click="MyRankPage" class="arrow-button left">&#60; 나의 랭킹</button>
      <!-- 오른쪽 화살표 버튼 -->
      <button @click="FriendRankPage" class="arrow-button right">친구 랭킹 &#62;</button>
    </div>
  </div>

  <div class="rank">
    <div class="green-rank" v-for="(family, index) in sortedFamilys" :key="index">
...
export default {
  ...
  computed: {
    sortedFamilys() {
      ...
      const sortedArr = [...this.data[2].familys].sort((a, b) => b.range - a.range);
      return sortedArr;
    }
  }
}
