<template>
  <div>
    ...
    <div class="arrow-buttons-container">
      <!-- 왼쪽 화살표 버튼 -->
      <button @click="FriendRankPage" class="arrow-button left">&#60; 친구 랭킹</button>
      <!-- 오른쪽 화살표 버튼 -->
      <button @click="MyRankPage" class="arrow-button right">나의 랭킹 &#62;</button>
    </div>
  </div>

  <div class="rank">
    <h3 class="top">TOP 10</h3>
      <div class="green-rank" v-for="(pet, index) in sortedPets.slice(0, 10)" :key="index">
...
export default {
  ...
  computed: {
    sortedPets() {
      ...
      const sortedArr = [...this.data[3].pets].sort((a, b) => b.level - a.level);
      return sortedArr;
    }
  }
}
